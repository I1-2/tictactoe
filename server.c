...
/* Czekanie nie dłużej niż sekund. */
    ...
    if (FD_ISSET(fdListen, &rfds)) {
        ...
        for (polIdx = 0; polIdx < 10; polIdx++) {
            if (mojePolaczenia[polIdx].activ == 0) { //Przyjmowanie nowego połaczenia
                mojePolaczenia[polIdx].activ = 1;
                mojePolaczenia[polIdx].fd = accept(fdListen, NULL, NULL);
                printf("Nowe polaczenie\n");
                break;
            }
        }
        if (polIdx == 10) {  //Serwer obsługuje 10 połączeń, odrzucanie połączenia
    ...
    if (FD_ISSET(mojePolaczenia[i].fd, &rfds)) {
        ...
        if (odczytano < 1) { //Zakończenie połączenia
            mojePolaczenia[i].activ = 0;
            close(mojePolaczenia[i].fd);
            mojePolaczenia[i].fd = -1;
        } else {
            printf("Odebrałem od klienta %d: %s", i, buf);
            int polIdx;
            for (polIdx = 0; polIdx < 10; polIdx++) { //Rozsyłanie wiadomości do pozostałych połączeń
                if (polIdx == i)
                    continue;
                if (mojePolaczenia[polIdx].activ)
